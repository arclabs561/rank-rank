...
- ✅ rank-soft has `differentiable_topk()` but uses sigmoid, not Gumbel
...
**Status**: ✅ **Implemented** - Available in `rank-soft` with `gumbel` feature
...
- ✅ Documentation: `rank-soft/docs/GUMBEL_RERANKING.md`

**Usage**:
```bash
cargo add rank-soft --features gumbel
cargo run --example gumbel_reranking --features gumbel
```

## rank-retrieve: First-Stage Retrieval

### LTRGR: Learning to Rank in Generative Retrieval (AAAI 2024) ⭐ **Highly Relevant**

**Paper**: "Learning to Rank in Generative Retrieval"  
**Authors**: Yongqi Li, Nan Yang, Liang Wang, Furu Wei, Wenjie Li  
**Venue**: AAAI 2024 (arXiv:2306.15222v2)  
**GitHub**: https://github.com/liyongqi67/LTRGR

**Summary**: Bridges the gap between generative retrieval (which generates passage identifiers) and the final passage ranking objective by adding a learning-to-rank training phase. Uses margin-based rank loss to optimize autoregressive models toward optimal passage ranking order.

**Key Techniques**:
- Two-phase training: learning-to-generate (standard generative retrieval) + learning-to-rank (margin-based loss)
- Multiview identifiers: title, substring, pseudo-query representations
- FM-index for constrained generation (ensures valid identifiers)
- Margin-based rank loss: `L_rank = max(0, s(q, p_n) - s(q, p_p) + m)`
- Multi-task loss: combines generation loss + two rank losses with different sampling strategies

**Connection to rank-retrieve**:
- ✅ Generative retrieval is a retrieval method (fits in rank-retrieve)
- ⏳ Identifier generation (titles, substrings, pseudo-queries) - new module needed
- ⏳ FM-index integration for constrained generation - new dependency
- ⏳ Heuristic scoring function (converts identifiers to passage scores) - new module
- ⏳ LTR training integration - could use rank-learn or keep self-contained

**Connection to rank-learn**:
- ⏳ Margin-based rank loss implementation
- ⏳ Multi-task training (generation + ranking losses)
- ⏳ Two-phase training framework

**Connection to rank-soft**:
- ⏳ Could use differentiable ranking operations for rank loss
- ⏳ Soft ranking for passage scoring

**Results**:
- **Natural Questions**: 68.8 hits@5 (4.56% improvement over MINDER)
- **TriviaQA**: 70.2 hits@5 (2.63% improvement)
- **MS MARCO**: 40.2 R@5, 25.5 MRR@10 (36.3% improvement over MINDER)

**Status**: 📋 **Documented** - Implementation opportunity identified

**Implementation Plan**:
- See `crates/rank-retrieve/docs/LTRGR_IMPLEMENTATION.md` for detailed plan
- Priority: Medium (novel approach, requires autoregressive model integration)

**Key Insight**: Generative retrieval learns to generate identifiers as an intermediate goal, but the disconnect between generation objective and ranking target creates a learning gap. LTRGR bridges this by directly optimizing for passage ranking.

## rank-rerank: Late Interaction Scoring
...
**Connection**: rank-rerank implements MaxSim (ColBERT-style token-level similarity)
...
- **Pipeline**: Dense → MaxSim (rank-rerank) → Cross-encoder (mxbai-rerank)

**When to use which**:
| Approach | Candidates | Speed | Quality | Pre-computation |
|----------|-----------|-------|---------|-----------------|
| MaxSim (rank-rerank) | 100-1000 | Fast (~61ms) | Good | Needs token embeddings |
| mxbai-rerank | 10-100 | Medium (~0.9s) | Best | Raw text input |

**Status**: ✅ **Integrated** - Tested integration with evaluation

**Examples**:
- `rank-rerank-python/examples/two_stage_reranking.py` - Basic pipeline demo
- `rank-rerank-python/examples/comprehensive_reranking_eval.py` - Full eval with metrics
...
**Installation**:
```bash
pip install mxbai-rerank rank-rerank rank-eval
```
...
**Status**: ✅ **Integrated** - See `rank-rerank-python/examples/flashrank_integration.py`
...
**Integration Opportunity**: Add rank-rerank as a backend option

**Status**: ✅ **Integrated** - See `rank-rerank-python/examples/rerankers_unified.py`
...
**Connection to rank-soft**: Could add as differentiable loss function
...
**Status**: ✅ **Implemented** in rank-soft
...
**Status**: ✅ **Implemented** in `rank-rerank-python/rank_rerank/rag_dynamic.py`
...
**Status**: ✅ **Implemented** in `rank-rerank-python/rank_rerank/multimodal.py`
...
**Status**: ✅ **Implemented** in `rank-rerank-python/rank_rerank/llm_rerank.py`
...
**Status**: ✅ **Implemented** in `rank-rerank-python/rank_rerank/onnx_export.py`
