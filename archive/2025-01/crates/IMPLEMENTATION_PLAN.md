...
```
rank[i] = (Σ_{j≠i} sigmoid(α * (values[i] - values[j]))) / valid_comparisons * (n-1)

∂rank[i]/∂values[k] = {
  if i == k: (α / valid_comparisons) * (n-1) * Σ_{j≠i} sigmoid'(α * (values[i] - values[j]))
  if i != k: -(α / valid_comparisons) * (n-1) * sigmoid'(α * (values[i] - values[k]))
}
where valid_comparisons is the number of finite values excluding self
```
