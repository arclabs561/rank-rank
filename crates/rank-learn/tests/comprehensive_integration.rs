...
mod tests {
    ...
    fn test_lambdarank_with_different_sigma() {
        ...
        let lambdas_default = trainer_default.compute_gradients(&scores, &relevance, None).unwrap();
        let lambdas_small = trainer_small_sigma.compute_gradients(&scores, &relevance, None).unwrap();
        let lambdas_large = trainer_large_sigma.compute_gradients(&scores, &relevance, None).unwrap();
    ...
    fn test_lambdarank_gradient_magnitude() {
        ...
        let perfect_lambdas = trainer.compute_gradients(&perfect_scores, &perfect_relevance, None).unwrap();
        ...
        let wrong_lambdas = trainer.compute_gradients(&wrong_scores, &wrong_relevance, None).unwrap();
