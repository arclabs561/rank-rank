...
mod tests {
    ...
    fn test_large_list_ndcg_performance() {
        ...
        assert!(
            ...
            "NDCG computation took too long: {:?}",
            elapsed
        );
    ...
    fn test_large_list_lambdarank_performance() {
        ...
        assert!(
            ...
            "LambdaRank computation took too long: {:?}",
            elapsed
        );
    ...
    fn test_batch_processing_performance() {
        ...
        let batches: Vec<(Vec<f32>, Vec<f32>)> = (0..batch_size)
            .map(|_| {
                ...
                let relevance: Vec<f32> = (0..list_size)
                    .map(|i| ((list_size - i) as f32) * 0.1)
                    .collect();
                (scores, relevance)
            })
            .collect();
    ...
    fn test_ndcg_repeated_computation() {
        ...
        assert!(
            (max_time as f64) < avg_time * 20.0 || avg_time < 0.001,
            "Performance inconsistency detected: max={}ns, avg={:.2}ns",
            max_time,
            avg_time
        );
    }
        ...
        for n in [10, 50, 100, 500] {
            ...
            if n == 500 {
                assert!(
                    ...
                    "LambdaRank for n={} took too long: {:?}",
                    n,
                    elapsed
                );
            }
