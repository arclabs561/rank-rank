...
fn main() {
    ...
    match retrieve_dense(&dense, &[1.0, 0.0], 10) { // 2D query
        Ok(results) => println!("   Retrieved {} documents", results.len()),
        Err(RetrieveError::DimensionMismatch { query_dim, doc_dim }) => {
            println!("   ✓ Correctly caught: Query has {} dimensions, document has {}", query_dim, doc_dim);
        }
        Err(e) => println!("   Unexpected error: {}", e),
    }
    ...
    match retrieve_bm25(&index, &query, 10, Bm25Params::default()) {
        ...
        Err(RetrieveError::DimensionMismatch { query_dim, doc_dim }) => {
            eprintln!("   Error: Dimension mismatch (query: {}, doc: {})", query_dim, doc_dim);
        }
        Err(e) => {
