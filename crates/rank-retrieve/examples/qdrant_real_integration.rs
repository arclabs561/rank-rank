        ...
        .map(|(i, point)| {
            let id = point
                ...
                .and_then(|id| match id {
                    qdrant_client::qdrant::PointId { point_id_options } => match point_id_options {
...
fn search_qdrant_mock(_url: &str, _query_embedding: &[f32], top_k: usize) -> Vec<(String, f32)> {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dense_results = {
        #[cfg(feature = "qdrant")]
        {
            ...
            let results =
                futures::executor::block_on(search_qdrant_real(&client, &query_embedding, 100))?;
    ...
    let top_candidates: Vec<(String, Vec<Vec<f32>>)> = fused
        ...
        .map(|(id, _)| {
            ...
            let doc_tokens = vec![vec![0.9, 0.1, 0.0], vec![0.1, 0.9, 0.0]];
            (id.clone(), doc_tokens)
        })
        .collect();
