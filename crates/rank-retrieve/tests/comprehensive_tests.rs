...
fn test_concrete_vs_trait_consistency() {
    ...
    assert!(concrete_results.iter().all(|(_, score)| score.is_finite() && *score >= 0.0));
}
...
fn test_retrieval_consistency_across_calls() {
    ...
    for (id, score1) in &map1 {
        ...
        assert!((score1 - score2).abs() < 1e-5, "Scores should be consistent for doc {}: {} vs {}", id, score1, score2);
    }
}
...
fn test_results_sorted_descending() {
    ...
    for i in 1..results.len() {
        assert!(results[i-1].1 >= results[i].1, "Results must be sorted descending");
    }
}
...
fn test_batch_large_number_queries() {
    ...
    let queries: Vec<Vec<String>> = (0..1000)
        .map(|i| vec![format!("term{}", i % 10)])
        .collect();
