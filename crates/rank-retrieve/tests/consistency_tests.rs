...
#[cfg(feature = "dense")]
use rank_retrieve::{retrieve_dense, batch::batch_retrieve_dense};
#[cfg(feature = "dense")]
use rank_retrieve::dense::DenseRetriever;
#[cfg(feature = "sparse")]
use rank_retrieve::{retrieve_sparse, batch::batch_retrieve_sparse};
#[cfg(feature = "sparse")]
...
fn test_repeatable_read_isolation() {
    ...
    for (id, score1) in &map1 {
        if let Some(score2) = map2.get(id) {
            assert!((score1 - score2).abs() < 1e-6, "Scores should be identical for doc {}", id);
        }
    }
    
    // Verify that at least some documents are common (not all ties)
    let common: Vec<u32> = map1.keys().filter(|id| map2.contains_key(id)).cloned().collect();
    assert!(!common.is_empty(), "At least some documents should be common");
}
...
fn test_no_phantom_reads() {
    ...
    assert_eq!(ids1, ids2, "Result sets should be identical (no phantom reads)");
}
...
fn test_concurrent_reads_consistency() {
    ...
    for other in &all_results[1..] {
        ...
        for (id, score1) in &first_map {
            let score2 = other_map.get(id).expect("Document should be in all results");
...
fn test_score_determinism() {
    ...
    for score in &scores[1..] {
        assert!((first_score - score).abs() < 1e-6, "Scores should be deterministic");
    }
}
...
fn test_batch_operation_consistency() {
    ...
    for (i, results) in batch_results.iter().enumerate() {
        ...
        let individual_map: std::collections::HashMap<u32, f32> = individual.iter().cloned().collect();
        
        assert_eq!(batch_map.len(), individual_map.len());
        for (id, score1) in &batch_map {
            let score2 = individual_map.get(id).expect("Document should be in both results");
            assert!((score1 - score2).abs() < 1e-6);
        }
    }
}
    ...
    for other in &all_results[1..] {
        ...
        for (id, score1) in &first_map {
            if let Some(score2) = other_map.get(id) {
                assert!((score1 - score2).abs() < 1e-5, "Scores should match for doc {}", id);
            }
        }
        
        // Verify at least some documents are common (not all ties)
        let common: Vec<u32> = first_map.keys().filter(|id| other_map.contains_key(id)).cloned().collect();
        assert!(!common.is_empty(), "At least some documents should be common across results");
    }
}
