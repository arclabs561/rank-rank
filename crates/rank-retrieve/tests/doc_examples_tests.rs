...
mod tests {
    ...
    fn test_token_pooling_research_claims() {
        ...
        assert!(reduction_2 >= 0.4 && reduction_2 <= 0.6, 
                "Factor 2 should reduce by ~50%, got {:.1}%", reduction_2 * 100.0);
        ...
        assert!(reduction_3 >= 0.5 && reduction_3 <= 0.75, 
                "Factor 3 should reduce by ~66%, got {:.1}%", reduction_3 * 100.0);
    ...
    fn test_pooling_index_time_claim() {
        ...
        assert!(score_full_query >= score_pooled_query,
