...
mod tests {
    ...
    use rank_fusion::{combsum, rrf, rrf_multi, RrfConfig};
    ...
    fn test_concrete_functions_with_fusion() {
        ...
        assert!(fused
            .iter()
            .all(|(id, score)| { *id < 3 && score.is_finite() && *score >= 0.0 }));
    ...
    fn test_batch_retrieval_e2e() {
        ...
        let batch_results =
            batch::batch_retrieve_bm25(&bm25_index, &queries, 5, Bm25Params::default()).unwrap();

        assert_eq!(batch_results.len(), 3);

        // Fuse results from multiple queries
        let fused = rrf_multi(
            &[&batch_results[0], &batch_results[1], &batch_results[2]],
            RrfConfig::default(),
        );
    ...
    fn test_retrieve_fuse_rerank_pipeline() {
        ...
        let candidates: Vec<Candidate<u32>> = fused
            .iter()
            .map(|(id, score)| {
                let embedding = doc_embeddings
                    .get(*id as usize)
                    .map(|e| e.as_slice())
    ...
    fn test_multi_fusion_methods() {
        ...
        assert!(rrf_fused
            .iter()
            .all(|(id, score)| { *id < 2 && score.is_finite() && *score >= 0.0 }));
        assert!(combsum_fused
            .iter()
            .all(|(id, score)| { *id < 2 && score.is_finite() && *score >= 0.0 }));
    }
    ...
    fn test_large_scale_retrieval_fusion() {
        ...
        let bm25_results =
            retrieve_bm25(&bm25_index, &query_terms, 50, Bm25Params::default()).unwrap();
    ...
    fn test_output_format_consistency() {
        ...
        assert!(bm25_results
            .iter()
            .all(|(id, score)| { *id < 10 && score.is_finite() }));
