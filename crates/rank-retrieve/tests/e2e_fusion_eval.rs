...
mod tests {
    ...
    fn test_complete_pipeline_real() {
        ...
        bm25_index.add_document(0, &["machine", "learning", "tutorial"].iter().map(|s| s.to_string()).collect::<Vec<_>>());
        bm25_index.add_document(1, &["deep", "learning", "neural"].iter().map(|s| s.to_string()).collect::<Vec<_>>());
        bm25_index.add_document(2, &["python", "programming"].iter().map(|s| s.to_string()).collect::<Vec<_>>());
    ...
    fn test_pipeline_data_flow() {
        ...
        assert!(retrieved.iter().all(|(doc_id, score)| {
            *doc_id < 10 && score.is_finite() && *score >= 0.0
        }));
