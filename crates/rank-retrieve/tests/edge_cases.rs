...
use rank_retrieve::generative::{GenerativeRetriever, HeuristicScorer, MockAutoregressiveModel};
#[cfg(feature = "sparse")]
...
fn bm25_empty_query() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        rank_retrieve::RetrieveError::EmptyQuery
    ));
}
...
fn bm25_empty_index() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        rank_retrieve::RetrieveError::EmptyIndex
    ));
}
...
fn bm25_zero_k() {
    ...
    let results = index
        .retrieve(&["test".to_string()], 0, Bm25Params::default())
        .unwrap();
...
fn bm25_single_term_document() {
    ...
    let results = index
        .retrieve(&["test".to_string()], 10, Bm25Params::default())
        .unwrap();
...
fn dense_mismatched_dimensions() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        rank_retrieve::RetrieveError::DimensionMismatch { .. }
    ));
}
...
fn generative_case_insensitive_scorer() {
    ...
    let mut retriever = GenerativeRetriever::new(model).with_scorer(scorer);
...
fn generative_large_beam_size() {
    let model = MockAutoregressiveModel::new();
    let mut retriever = GenerativeRetriever::new(model).with_beam_size(100);
