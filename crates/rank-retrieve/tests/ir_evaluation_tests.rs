...
fn test_three_way_method_comparison() {
    ...
    let docs = vec![
        (
            ...
            vec![1.0, 0.0, 0.0],
            SparseVector::new_unchecked(vec![0, 1], vec![1.0, 0.8]),
        ),
        (
            1,
            vec!["deep".to_string(), "learning".to_string()],
            vec![0.9, 0.1, 0.0],
            SparseVector::new_unchecked(vec![1, 2], vec![1.0, 0.6]),
        ),
        (
            2,
            vec!["python".to_string(), "programming".to_string()],
            vec![0.0, 1.0, 0.0],
            SparseVector::new_unchecked(vec![2, 3], vec![1.0, 0.4]),
        ),
    ];
    ...
    let bm25_ranked: Vec<String> = bm25_results.iter().map(|(id, _)| id.to_string()).collect();
    let dense_ranked: Vec<String> = dense_results.iter().map(|(id, _)| id.to_string()).collect();
    let sparse_ranked: Vec<String> = sparse_results
        .iter()
