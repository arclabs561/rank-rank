...
mod tests {
    ...
    fn test_large_index_performance() {
        ...
        assert!(elapsed.as_secs_f64() < 5.0, "Indexing took too long: {:?}", elapsed);
    }
    ...
    fn test_large_retrieval_performance() {
        ...
        assert!(elapsed.as_millis() < 100, "Retrieval took too long: {:?}", elapsed);
    ...
    fn test_dense_large_scale() {
        ...
        assert!(elapsed.as_millis() < 500, "Dense retrieval took too long: {:?}", elapsed);
        assert!(!results.is_empty());
    }
    ...
    fn test_repeated_retrieval_consistency() {
        ...
        let avg_time: f64 = times.iter().map(|t| t.as_millis() as f64).sum::<f64>() / times.len() as f64;
        ...
        assert!((max_time as f64) < avg_time * 5.0 || avg_time < 1.0, 
            "Performance inconsistency detected: max={}ms, avg={:.2}ms", max_time, avg_time);
    }
