...
fn test_lexical_vs_semantic_performance() {
    ...
    let _semantic_results = retrieve_bm25(&index, &semantic_query, 10, Bm25Params::default()).unwrap();
...
fn test_batch_mixed_query_types() {
    ...
    let queries = vec![
        ...
        vec!["term0".to_string(), "term1".to_string(), "term2".to_string(), "term3".to_string(), "term4".to_string()], // Long query
    ];
...
fn test_query_type_evaluation() {
    ...
    // Long query
    let long_query = vec!["machine".to_string(), "learning".to_string(), "algorithms".to_string()];
