...
use rank_eval::binary::{precision_at_k, recall_at_k, ndcg_at_k};
...
fn test_no_answer_empty_results() {
    ...
    let query = vec!["completely".to_string(), "unrelated".to_string(), "terms".to_string()];
    let results = retrieve_bm25(&index, &query, 10, Bm25Params::default()).unwrap();
...
fn test_dense_robustness_zero_vector() {
    ...
    assert!(!results.is_empty());
    assert!(results.iter().all(|(_, score)| *score == 0.0 || score.is_finite()));
}
