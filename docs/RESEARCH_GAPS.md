...
- LambdaLoss, ApproxNDCG, RankNet added to rank-relax
- FlashRank integration example in rank-refine-python
...
### Currently Supported in rank-relax
...
**Integration opportunity**: Add rank-refine as a backend:
```python
ranker = Reranker('rank-refine', model_type='maxsim')
```
...
2. ~~Add LambdaLoss to rank-relax~~ - Plus RankNet, ApproxNDCG, ListNet, ListMLE
...
1. Add CrossEncoderModel trait to rank-refine (native Rust implementation)
